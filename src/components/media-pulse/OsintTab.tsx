        ...
        {paged.map(r => (
          <a key={r.id} href={r.url} target="_blank" rel="noreferrer" className="border border-border rounded-xl p-4 hover:border-primary/50 transition-colors bg-muted/40">
            ...
            <div className="flex flex-wrap gap-2 mt-3 text-[11px] font-bold uppercase tracking-widest">
              ...
              {r.labels.map(l => <span key={l} className="px-2 py-1 rounded bg-muted text-foreground border border-border">{l}</span>)}
              {r.freeTier && <span className="px-2 py-1 rounded bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-400 border border-emerald-500/30">{t('badges.free')}</span>}
              {r.region && <span className="px-2 py-1 rounded bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-400 border border-indigo-500/30">{r.region}</span>}
            </div>
