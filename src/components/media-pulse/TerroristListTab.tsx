...
import {
  ...
  FileText, Upload, Trash2, Clock, XCircle,
  Filter, Download, X,
...
export default function TerroristListTab() {
  ...
  const [deleteConfirmationId, setDeleteConfirmationId] = useState<Id<'local_terrorist_list'> | null>(null);

  // Multi-Selection State
  const [selectedIds, setSelectedIds] = useState<Set<Id<'local_terrorist_list'>>>(new Set());
  ...
  const deleteMatching = useMutation(api.terroristList.deleteMatching);
  ...
  const addMatchingToCollection = useMutation(api.terroristList.addMatchingToCollection);
  const settings = useQuery(api.settings.getSettings);

  // Memoized check Set to optimize lookups
  const selectedSet = useMemo(() => selectedIds, [selectedIds]);

  // ─── Multi-Select Handlers ──────────────────────────────────────────────
  const toggleSelection = (id: Id<'local_terrorist_list'>) => {
    setSelectedIds(prev => {
  ...
  const toggleAll = () => {
    ...
    setSelectedIds(prev => {
      ...
      return new Set(entries.map(e => e._id));
    });
  };
  ...
  const handleExport = async (format: 'pdf' | 'excel') => {
    ...
    try {
      if (selectAllMode) {
        // Run imperative query to fetch ALL matching records from the database
        dataToExport = await convex.query(api.terroristList.searchAll, {
          searchTerm: searchQuery,
          type: filterType === 'all' ? undefined : filterType,
        });
  ...
  const handleSaveRecord = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingRecord?._id) {
        await updateItem({
          id: editingRecord._id as Id<'local_terrorist_list'>,
          type: editingRecord.type as 'individual' | 'entity' | 'organization' | undefined,
        ...
        await addSingleItem({
          type: (editingRecord?.type as 'individual' | 'entity' | 'organization') || 'individual',
  ...
  const handleDeleteRecord = async () => {
    ...
    try {
      await deleteItem({ id: deleteConfirmationId });
  ...
  const handleBulkDelete = async () => {
    if (!isAdmin || (selectedIds.size === 0 && !selectAllMode)) return;
    ...
    try {
      if (selectAllMode) {
        await deleteMatching({
          searchTerm: searchQuery,
          type: filterType === 'all' ? undefined : filterType,
          excludedIds: Array.from(selectedIds),
        });
      } else {
        await deleteItems({ ids: Array.from(selectedIds) });
      }
  ...
  const handleSaveToCollection = async () => {
    if (!collectionName.trim() || (selectedIds.size === 0 && !selectAllMode)) return;
    setBulkActionLoading(true);
    try {
      const collectionId = await createCollection({ name: collectionName.trim() });
      if (selectAllMode) {
        await addMatchingToCollection({
          collectionId,
          searchTerm: searchQuery,
          type: filterType === 'all' ? undefined : filterType,
          excludedIds: Array.from(selectedIds),
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Calculate total rows first to calculate total chunks
      let totalRowsCount = 0;
      workbook.eachSheet(sheet => {
        if (sheet.rowCount > 1) {
          totalRowsCount += (sheet.rowCount - 1);
        }
      });
      ...
      const chunkSize = 200;
      const totalChunks = Math.ceil(totalRowsCount / chunkSize);
      setImportProgress({ current: 0, total: totalChunks });

      let chunk: Partial<TerroristListItem>[] = [];
      let totalAdded = 0;
      let totalSkipped = 0;

      // Stream rows in chunks to prevent high memory usage
      for (const sheet of workbook.worksheets) {
        const headerRow = sheet.getRow(1);
        const headers = (headerRow.values as (string | number | boolean | null)[])
          .slice(1)
        ...
        for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber++) {
          ...
          const getVal = (valKeys: string[]) => {
            ...
            for (const key of valKeys) {
              ...
              if (foundKey && String(rowData[foundKey]).trim()) {
                let val = String(rowData[foundKey]).trim();
          ...
          if (nameAr || nameEn) {
            chunk.push({
              ...
              documentNumber: getVal(['رقم القضية', 'رقم الوثيقة', 'رقم الجواز', 'رقم الهوية', 'الرقم المدني', 'Document Number', 'ID', 'Passport', 'Identifier', 'Doc #']),
    ...
    <div className="space-y-6">
      ...
      {entries && entries.length > 0 && (
        <div className="space-y-2">
          {selectedSet.size === entries.length && !selectAllMode && status === "CanLoadMore" && (
            <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 text-center text-xs font-bold text-foreground space-x-2">
              <span>Selected all {entries.length} loaded records.</span>
              <button
                onClick={() => {
                  setSelectAllMode(true);
                  setSelectedIds(new Set());
                }}
                className="text-primary hover:underline font-extrabold uppercase tracking-wider text-[10px]"
              >
                Select all matching records in database
              </button>
            </div>
          )}
          {selectAllMode && (
            <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 text-center text-xs font-bold text-foreground space-x-2">
              <span>Selected all matching records in database {selectedSet.size > 0 && `(except ${selectedSet.size} unselected)`}.</span>
              <button
                onClick={() => {
                  setSelectAllMode(false);
                  setSelectedIds(new Set());
      ...
      {/* ─── Bulk Actions Bar ─── */}
      <AnimatePresence>
        {(selectedSet.size > 0 || selectAllMode) && (
          <motion.div
          ...
          >
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-primary/20 text-primary font-bold">
                {selectAllMode ? 'All' : selectedSet.size}
              </div>
              <span className="text-sm font-bold text-foreground">
                {selectAllMode ? 'All matching records selected' : `Selected ${selectedSet.size === 1 ? 'Record' : 'Records'}`}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Button
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-start border-collapse min-w-[1400px]">
                ...
                <tbody className="divide-y divide-border/50">
                  {entries?.map((entry) => {
                    const isChecked = selectAllMode ? !selectedSet.has(entry._id) : selectedSet.has(entry._id);
                    return (
                      <tr
                        key={entry._id}
                      ...
                      >
                        <td className="px-4 py-3 sticky start-0 bg-card group-hover:bg-muted/30 shadow-[2px_0_5px_rgba(0,0,0,0.05)] border-e border-border/60 z-10 w-[40px]">
                          <input
                            ...
                            aria-label={`Select record: ${entry.nameArabic || entry.nameLatin || 'unnamed'}`}
                            className="rounded border-border text-primary focus:ring-primary/20 bg-card cursor-pointer"
                          />
                        </td>
              ...
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    isChecked ? "border-primary ring-1 ring-primary/20 shadow-primary/10" : "border-border hover:border-primary/50"
                  )}
                >
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-foreground/70">
                  {selectAllMode 
                    ? "هل أنت متأكد من حذف جميع العناصر المطابقة؟ لا يمكن التراجع عن هذا الإجراء." 
                    : `هل أنت متأكد من حذف ${selectedSet.size} عنصر محدد؟ لا يمكن التراجع عن هذا الإجراء.`}
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-foreground/70 font-medium">
                      {selectAllMode 
                        ? "سيتم حفظ جميع العناصر المطابقة في القائمة." 
                        : `سيتم حفظ ${selectedSet.size} عنصر في القائمة.`}
                    </p>
                  </div>
                </div>
                <button
