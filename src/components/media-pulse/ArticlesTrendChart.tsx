...
export default function ArticlesTrendChart({ data }: ArticlesTrendChartProps) {
    ...
    useEffect(() => {
        ...
        const formatColor = (val: string, fallback: string) => {
            ...
            // If the CSS variable is just raw HSL channel values like "220 50% 50%", wrap it in hsl()
        ...
        setColors({
            primary: formatColor(getCSSVar("--primary"), "#2563EB"),
            border: formatColor(getCSSVar("--border"), "#E2E8F0"),
            mutedFg: formatColor(getCSSVar("--muted-foreground"), "#374151"),
            popover: formatColor(getCSSVar("--popover"), "#FFFFFF"),
            popoverFg: formatColor(getCSSVar("--popover-foreground"), "#020617"),
        });
    }, []);

    if (!mounted) {
        return (
            <div className="w-full flex items-end gap-1 px-2 pb-4 animate-pulse" style={{ height: "160px", minHeight: "160px", width: "100%" }}>
    ...
    return (
        <div className="w-full -mx-1" style={{ height: "160px", minHeight: "160px", width: "100%" }}>
            {mounted && (
